...
_gbg_get_workspace_status() {
    ...
    gbg_workspace_has_modifications=$( \
        [ "${gbg_workspace_modifications_num}" -gt 0 ] && \
        echo "true" || \
        echo "false" )

    gbg_workspace_deletions_num=$(echo "${lgbg_git_status}" | grep -Ece '^.D')
    gbg_workspace_has_deletions=$( \
        [ "${gbg_workspace_deletions_num}" -gt 0 ] && \
        echo "true" || \
        echo "false" )
    ...
    gbg_workspace_has_untracked=$( \
        [ "${gbg_workspace_untracked_num}" -gt 0 ] && \
        echo "true" || \
        echo "false" )

    gbg_workspace_ignored_num=$(echo "${lgbg_git_status}" | grep -Ece '^!!')
    gbg_workspace_has_ignored=$( \
        [ "${gbg_workspace_ignored_num}" -gt 0 ] && \
        echo "true" || \
        echo "false" )
